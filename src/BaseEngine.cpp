...
/*
This function calls SDL functions, and the aim of this coursework is to hide their complexity from you so you probably just want to call this and not worry about what it does.
...
int BaseEngine::initialise(const char* szCaption, int iWindowWidth, int iWindowHeight, const char* szFontName, int iFontSize)
{
    ...
    // Set the hint to shut down D-Bus connection on SDL_Quit.
...
void BaseEngine::deinitialise(void)
{
    ...
    SDL_DestroyWindow(m_pSDL2Window);
    SDL_DestroyRenderer(m_pSDL2Renderer);
    SDL_DestroyTexture(m_pSDL2Texture);
