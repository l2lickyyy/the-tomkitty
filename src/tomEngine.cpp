...
// Background Function
void tomEngine::virtSetupBackgroundBuffer() {
    // debug
...
void tomEngine::virtMainLoopDoBeforeUpdate() {
    if (counter >= 1000) {
        counter = 0;
    }
...
void tomEngine::virtDrawStringsOnTop() {
    // breathing effects
...
// Keyboard Function
void tomEngine::virtKeyDown(int iKeyCode) {
    switch (iKeyCode) {
    case SDLK_ESCAPE:
        if (gameState == STATE_MENU) {
...
// Mouse Function
void tomEngine::virtMouseDown(int iButton, int iX, int iY) {
...
// Object Function
int tomEngine::virtInitialiseObjects() {
    ...
    if (useKeyboard) {
        ...
        if (obj && dynamic_cast<tomObject*>(obj)) {
            static_cast<tomObject*>(obj)->setMouseControl(false);
        }
    }
...
// draw text outline
void tomEngine::drawTextWithOutline(int x, int y, const char* text, int colour, DrawingSurface* surface, Font* font) {
    // draw outline
...
void tomEngine::markToDestroy(DisplayableObject* obj){
    objectToDestroy.push_back(obj);
}

// destroy object
void tomEngine::destroyObject() {
...
int tomEngine::getLastFrametime() {
