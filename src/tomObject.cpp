...
void tomObject::virtDoUpdate(int iCurrentTime) {
	if (static_cast<tomEngine*>(getEngine())->getGameState() == STATE_PAUSE) return;
    ...
    // collision detection
    foeObject* enemy = dynamic_cast<foeObject*>(getEngine()->getDisplayableObject(1));
